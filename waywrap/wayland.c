...
struct client_state *client_state_new()
{
	...
	wl_registry_add_listener(state->wl_registry, &state->registry_listener,
		state);
...
struct surface_state *surface_state_new(struct client_state *client_state,
	const char *title, unsigned int width,
	unsigned int height)
{
	...
	state->xdg_surface =
		xdg_wm_base_get_xdg_surface(client_state->xdg_wm_base, state->wl_surface);
	xdg_surface_add_listener(state->xdg_surface, &state->surface_listener, state);
	state->xdg_toplevel = xdg_surface_get_toplevel(state->xdg_surface);
	xdg_toplevel_add_listener(state->xdg_toplevel, &state->toplevel_listener,
		state);
	...
	zxdg_toplevel_decoration_v1_add_listener(state->decos, &state->deco_listener,
		NULL);
...
struct wl_buffer *draw_frame(struct surface_state *state)
{
	...
	struct wl_buffer *buffer = wl_shm_pool_create_buffer(
		...
		state->wl_shm_format); // WL_SHM_FORMAT_XRGB8888);
  ...
  for (int y = 0; y < height; ++y) {
	for (int x = 0; x < width; ++x) {
	                ...
	                if (x >= xx - 10 && x <= xx + 10  && y >= yy - 10 && y <= yy +
  10) { data[y * width +x] = 0xFFFF0000; continue;
	                }
	...
	const char *interface, uint32_t version)
{
	...
	if (strcmp(interface, wl_shm_interface.name) == 0) {
		...
		xdg_wm_base_add_listener(state->xdg_wm_base, &state->wm_base_listener,
			state);
	...
	uint32_t capabilities)
{
	...
	if (have_pointer && state->wl_pointer == NULL) {
		...
		wl_pointer_add_listener(state->wl_pointer, &state->pointer_listener, state);
	...
	if (have_keyboard && state->wl_keyboard == NULL) {
		...
		wl_keyboard_add_listener(state->wl_keyboard, &state->keyboard_listener,
			state);
